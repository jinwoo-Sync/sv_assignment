...
#### 1. 네트워킹 & 분산 처리
- [x] 비차단 I/O로 50개 이상 agent 동시 연결
- [x] agent 주기적 HEARTBEAT(1s) 및 STATE 보고
- [x] 메시지 무결성 검사 (CRC32)
- [ ] 헬스체크 타임아웃 (3s 미응답 시 Unhealthy)
- [ ] 자동 재시도/재연결 (지수 백오프)
- [ ] CMD_SET_MODE 브로드캐스트 및 부분 실패 보상 로직

#### 2. 시스템 제어 도메인 로직
- [ ] 정책 엔진 — 평균 부하 > 임계치 시 그룹 모드 변경
...
- [ ] IStateStore, ICommandBus, IPolicyEngine 인터페이스

#### 5. 테스트 기반 개발
- [x] 단위 테스트 — MemoryPool, Logger
- [ ] 단위 테스트 — 프로토콜 인코더/디코더, 정책 엔진, 타임아웃 로직
...
- [ ] 정상 플로우: CMD_SET_MODE(perf) 브로드캐스트 → ACK 수신
- [ ] 부분 실패: NACK/응답 지연 주입 → controller 재시도 후 폴백 로깅
- [ ] 장애/복구: agent 컨테이너 재시작 → controller 재연결 및 상태 동기화
